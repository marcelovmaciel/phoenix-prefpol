...
function strict_profile(x::Profile)
    is_strict(x) || throw(ArgumentError("Expected a strict PreferenceProfiles.Profile input."))
    return x
end

function strict_profile(x::WeightedProfile)
    is_strict(x) || throw(ArgumentError("Expected a strict PreferenceProfiles.WeightedProfile input."))
    return x
end
...
function strict_profile(profile::AbstractVector; candidate_syms = nothing)
    ...
    all(ranking -> ranking isa AbstractDict, profile) || throw(ArgumentError(
        "Expected a vector of ranking dictionaries or a strict PreferenceProfiles profile.",
    ))
