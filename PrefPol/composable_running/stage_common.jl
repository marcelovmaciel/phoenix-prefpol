...
const DEFAULT_MAIN_MEASURES = [:Psi, :R, :HHI, :RHHI, :C, :D]
...
function parse_args(args)
    ...
    while i <= length(args)
        ...
        elseif arg in ("--help", "-h")
            println("""
            Usage:
              julia --project=PrefPol PrefPol/composable_running/stages/<stage>.jl [--config PATH] [--year YEAR] [--scenario NAME] [--m VALUE_OR_RANGE] [--backend NAME] [--linearizer NAME] [--force] [--dry-run] [--smoke-test]

            Staged composable replication entry point.
            """)
            exit(0)
        else
...
function manifest_path_error(label::AbstractString, original::AbstractString,
    ...
    Rerun the upstream composable stage to rewrite manifests with portable paths, or rerun it with --force to regenerate missing caches.
    """
end
...
function selected_targets(cfg, opts)
    ...
    isempty(targets) && error("No targets selected for stage.")
    return targets
end
...
function build_batch(targets, wave_by_id, settings)
    ...
    isempty(items) && error("No stage batch items were generated.")
