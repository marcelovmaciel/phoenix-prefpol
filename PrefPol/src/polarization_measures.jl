...
function _strict_consensus_ballot(consensus, pool)
    if consensus isa ConsensusResult
        return consensus.consensus_ballot
    elseif consensus isa Preferences.StrictRank
        return consensus
    elseif consensus isa AbstractDict
        return _strict_ballot_from_dict(consensus; candidate_syms = Preferences.candidates(pool))[2]
    end
    throw(ArgumentError("Unsupported consensus type $(typeof(consensus)); expected ConsensusResult, Dict or Preferences.StrictRank."))
end
...
function pairwise_group_divergence(profile_i, consensus_j, m::Int)
    _candidate_set_matches(strict_profile(profile_i), consensus_j) || throw(ArgumentError(
...
_profile_candidate_tuple(profile::AbstractVector{<:AbstractDict}) = _profile_candidate_tuple(strict_profile(profile))
_profile_candidate_tuple(profile::Preferences.Profile) = _candidate_tuple(Preferences.candidates(profile.pool))
_profile_candidate_tuple(profile::Preferences.WeightedProfile) = _candidate_tuple(Preferences.candidates(profile.pool))
_profile_candidate_tuple(profile::AnnotatedProfile) = _profile_candidate_tuple(profile.profile)
...
function _assert_group_candidate_tuple_consistency(group_profiles, consensus_map)
    ...
    for consensus in values(consensus_map)
        ...
        elseif consensus isa Preferences.StrictRank
            length(Preferences.to_perm(consensus)) == length(expected) || throw(ArgumentError(
...
function overall_divergences(grouped_consensus, whole_df, key)
    ...
    consensus_col = if :consensus_result in cols
        :consensus_result
...
function _group_proportion_map(bundle::AnnotatedProfile, grouped_indices)
    if bundle.profile isa Preferences.WeightedProfile
        total = Float64(Preferences.total_weight(bundle.profile))
