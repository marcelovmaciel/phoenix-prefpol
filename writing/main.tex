...
Regarding the behavior of the group diagnostics \(C\), \(D\), \(S\), and \(O\), it is useful to interpret them against two reference profiles. First, consider an \(e\)-profile, where every possible ranking appears with equal frequency within each group. In this case, group labels do not organize the profile. The best consensus ranking still mismatches group members on about half of all pairwise comparisons, so \(C^{\star}\approx 1/2\) and the rebased coherence is \(C\approx 0\). The divergence statistic \(D\), however, need not be close to zero, because it compares group members to other groups' fitted consensuses; in an unstructured ranking space, these consensuses are still typically at nonzero Kendall distance from individual rankings. This is why \(D\) must be interpreted together with support separation. Since \(S=D-(1-C^{\star})\), the within-group dispersion term absorbs the generic distance induced by the lack of structure, yielding \(S\approx 0\). Overlap also diagnoses the same situation: if all groups approximate the same uniform distribution over rankings, then \(O\approx 1\), or equivalently \(1-O\approx 0\).
...
% \begin{figure}[htbp]
%     \raggedleft 
%   \noindent\includegraphics[ height=0.3\textheight, keepaspectratio]{imgs/2006_group.png}
...
non-overlap \(1-O\), and support separation \(S\). The main result is
...
signed support-separation measure \(S\) is the clearest indication of the
...
support separation. For income, education, ideology, and age,
...
\(1-O \approx 0.43\) to \(1-O \approx 0.53\), and its signed support separation
...
support separation. The 2018 profile is therefore not simply more conflictual than 2006; among the
...
moderate, and their signed support separation \(S\) remains close to zero or
...
support separation, especially for smaller candidate sets, but it declines as
...
the largest non-overlap \(1-O\), and the largest signed support separation
...
and high signed support separation.
...
divergence, non-overlap, and support separation, especially in the binary
...
non-overlap, and substantial support separation. Ideology is also informative,
...
or near-highest divergence, the largest non-overlap, and the largest support
...
\subsection{Decomposition of the Group Divergence Statistic}

This appendix demonstrates why the group divergence statistic \(D\) can be decomposed into within-group dispersion (\(1 - C^{\star}\)) plus an excess-distance component. It also proves why the support-separation statistic \(S\) is strictly non-negative.


Let \(A\) be the active candidate set, with \(|A|=m\), and let \(\mathcal{L}(A)\) denote the set of linear orders over \(A\). For each group \(G_g\) of size \(n_g\), we use the normalized average Kendall loss of a candidate consensus ranking \(\rho \in \mathcal{L}(A)\), defined as:
\[
\ell_g(\rho) = \frac{1}{n_g} \sum_{x \in G_g} \frac{d_\tau(x,\rho)}{\binom{m}{2}}
\]
where \(x\) represents the observed ranking of an individual member in \(G_g\).

The consensus set of group \(g\) consists of the ranking(s) that minimize this loss:
\[
M_g = \arg\min_{\rho \in \mathcal{L}(A)} \ell_g(\rho)
\]

To account for cases where \(M_g\) is not a singleton, all consensus-dependent quantities are averaged over the minimizing rankings. Let \(\bar{\ell}_g(M_h)\) denote the average loss of group \(g\)'s members relative to the consensus set of another group \(h\):
\[
\bar{\ell}_g(M_h) = \frac{1}{|M_h|} \sum_{\rho \in M_h} \ell_g(\rho)
\]


The within-group dispersion of \(G_g\) around its own consensus is its minimum average loss:
\[
W_g = \bar{\ell}_g(M_g) = \min_{\rho \in \mathcal{L}(A)} \ell_g(\rho)
\]
Because every ranking \(\rho \in M_g\) achieves this exact minimum, \(W_g\) is simply the optimal value \(\ell_g(\rho)\).

The aggregate within-group dispersion (\(W\)) is the size-weighted average across all \(k\) groups. By definition, this is the inverse of the raw coherence statistic \(C^{\star}\):
\[
W = \sum_{g=1}^k \frac{n_g}{n} W_g = 1 - C^{\star}
\]


The group divergence statistic measures the average distance from members of one group to the consensuses of \textit{other} groups:
\[
D = \frac{1}{k-1} \sum_{g=1}^k \sum_{h \neq g} \frac{n_g}{n} \bar{\ell}_g(M_h)
\]

For each ordered pair of distinct groups (\(g \neq h\)), we can decompose the distance by adding and subtracting group \(g\)'s own internal dispersion, \(W_g\):
\[
\bar{\ell}_g(M_h) = W_g + \left[ \bar{\ell}_g(M_h) - W_g \right]
\]

Substituting this decomposition into the equation for \(D\) yields two separate summations:
\[
D = \underbrace{\frac{1}{k-1} \sum_{g=1}^k \sum_{h \neq g} \frac{n_g}{n} W_g}_{\text{Internal Dispersion}} + \underbrace{\frac{1}{k-1} \sum_{g=1}^k \sum_{h \neq g} \frac{n_g}{n} \left[ \bar{\ell}_g(M_h) - W_g \right]}_{\text{Support Separation } (S)}
\]


In the first term, the value \(\frac{n_g}{n} W_g\) depends only on \(g\), not on \(h\). For any fixed group \(g\), there are exactly \((k-1)\) other groups where \(h \neq g\). Summing over \(h\) therefore multiplies the term by \((k-1)\), which cleanly cancels out the normalization constant:
\[
\frac{1}{k-1} \sum_{g=1}^k (k-1) \frac{n_g}{n} W_g = \sum_{g=1}^k \frac{n_g}{n} W_g = W = 1 - C^{\star}
\]


With the first term simplified, the decomposition of \(D\) becomes:
\[
D = (1 - C^{\star}) + S
\]
where \(S\) is the support-separation statistic:
\[
S = \frac{1}{k-1} \sum_{g=1}^k \sum_{h \neq g} \frac{n_g}{n} \left[ \bar{\ell}_g(M_h) - W_g \right]
\]
Equivalently, \(S = D - (1 - C^{\star})\).

\textbf{Proof of non-negativity:} By definition, \(W_g\) is the absolute minimum possible average loss for group \(g\) (\(\min_{\rho} \ell_g(\rho)\)). Therefore, for \textit{any} ranking \(\rho\) belonging to another group's consensus set \(M_h\), it must hold true that \(\ell_g(\rho) \ge W_g\).

Consequently, the average loss \(\bar{\ell}_g(M_h) \ge W_g\), meaning the bracketed difference is always non-negative:
\[
\bar{\ell}_g(M_h) - W_g \ge 0
\]
Because \(S\) is a size-weighted sum of strictly non-negative terms, it follows that:
\[
S \ge 0
\]

The statistic \(S\) is not a signed difference between unrelated quantities; it is the non-negative residual in the decomposition of \(D\). While internal dispersion (\(W\)) is naturally bounded (because a consensus can always be oriented to agree with at least half the group on any pair), the distance to \textit{another} group's consensus (\(\bar{\ell}_g(M_h)\)) is not restricted by this same internal limit. Thus, \(S\) captures the true excess distance separating groups after factoring out the internal dispersion already baked into \(D\).
...
\input{./imgs/appendix_lambda_grouping_tables.tex}
