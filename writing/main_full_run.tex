...
The statistic \(D\) is useful as a measure of external divergence, but it
is not a pure distance between group consensus rankings. Because it
compares individual rankings to other groups' consensus sets, internal
dispersion also contributes to \(D\). A group whose members are already
far from their own consensus will also tend to be far from other groups'
consensuses. Appendix~\ref{app:c-d-relationship} shows that
\(D
\geq
1-C^{\star}
=
...
a diagnostic pair. The goal is not to assign each partition one scalar value of
polarization, but to describe how, and in what sense, an exogenous grouping
structures the preference profile. In this sense, the paper extends the
...
substantively defined candidate sequence for each year.\footnote{The
  ...
  Appendix~\ref{app:candidate-set-selection}.} For each year, profiles with smaller
...
moving to \(m=6\) or \(m=7\) creates sparsity in the ranking data.

For each linearized pseudo-profile, I compute the global measures \(\Psi\),
\(R\), \(\kappa\), and \(RHHI\). I then compute the group-level measures \(C\)
