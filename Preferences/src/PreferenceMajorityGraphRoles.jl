...
Threshold parameters used by majority-graph role classification tables.
"""
...
Roles are defined relative to the majority graph induced by the profile; they are
not intrinsic properties of ranking types. Roles are non-exclusive, so a type can
simultaneously be, for example, an anchor and an edge breaker. Stable
`type_index` values are inherited from `VoterTypeBasis`.
"""
...
Return an edge-by-type role table with edge-specific breaker classifications.
"""
...
Summarize mass by non-exclusive role. Role masses are not a partition: the same
type can contribute to multiple rows. The `mixed` row counts only types with no
boolean role assigned.
"""
...
Summarize mass by `primary_role`. Unlike `role_mass_summary`, this forms a
partition of type mass.
"""
...
Return the top edge-breaking rows for a selected edge. If no edge is provided,
the weakest majority edge is selected by minimum normalized margin.
"""
...
Return a long table of group mass by `primary_role`. Primary roles partition
each group's type mass.
"""
...
Decompose group "power" into graph anchoring, edge support, net margin
contribution, and edge-breaking capacity. These are edge-relative quantities.
